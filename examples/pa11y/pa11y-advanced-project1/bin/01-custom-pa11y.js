...
// - Uses the "runners:" option to specify use of the *HTML CodeSniffer* ruleset for testing. 
//   No rules are ignored, so testing is done using the full range of rules. 
//
// - Uses the "urls:" option and tests against 10 embedded URLs that are hard-coded inside the
//   script, as opposed to pointing to an external sitemap file for URLs to test. 
//
// - Uses the "standards:" option configured to use WCAG AA rules for HTML CodeSniffer. The 
//   HTML CodeSniffer ruleset is set via the "runners:" option as described above.
//
// - In the absence of the -h switch on the command, this example writes the test results to 
//   the *command window* for each URL tested.//
//=============================================================================================
