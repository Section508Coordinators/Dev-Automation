//Updated: 01/18/2021
//Technology: Pa11y CLI JavaScript Test Automation
//How to Execute: Type "node 02_pa11y_BasicScript.js" from Git Bash Command window
//
//
// This test script is customized to work as follows. This script will:
//
//		[1] - access a server hosted web page
//
//		[2] - suppress testing with unwanted rules, such as for HTMLCS Notices and any 
//			  other test rules not considered TTv5-friendly by using the "ignore:" option
//
//		[3] - run rules for HTML Code Sniffer only by disabling the "runners" option because
//			  when a test runner is not specified in the code, Pa11y uses HTML CodeSniffer
//			  rules by default.
//-----------------------------------------------------------------------------------------------------
...
vWebSite="https://section508coordinators.github.io/BaselineTestPages2/test-cases/TC1005C001.html"
...
pa11y(vWebSite, {
	...
	ignore: [
			//Pa11y rules that are "Notices" only
			'notice',
			// aXe-core version 3.5 rules to ignore as of the anlays performed: 4/15/2020
			'accesskeys',
			'area-alt',
			'aria-allowed-role',
			'aria-hidden-body',
			...
			// HTML Code Sniffer rules version 2.5.1 to ignore as of the anlays performed: 4/15/2020
...
// *** IF YOU DON'T SPECIFY A TEST RUNNER, HTML CODESNIFFER RULES ARE USED BY DEFAULT
//	runners: [
//        'axe',
//        'htmlcs'
//	],		
...
reporter: [
    ...
    numIssues=results.issues.length;

console.log('');
console.log('There are '+numIssues+' errors detected on the '+vWebSite+ ' web site as follows:');
console.log('');
...
for (i = 0; i < results.issues.length; i++) { 
	...
	console.log('    Code: '+results.issues[i].code);
	console.log('    Message: '+results.issues[i].message);
	console.log('    Context: '+results.issues[i].context);
	console.log('    Selector: '+results.issues[i].selector);
	console.log(' ');
	console.log('------------------------------------');
	console.log(' ');
}
